...
async def process_chunk(chunk, start_idx, total_records, model_client, termination_condition):
    ...
    處理單一批次資料：
      ...
      - 收集所有回覆訊息並返回。
    """
    # 將資料轉成 dict 格式（將team裡聊天後的結果用下方的文字呈現出來）
    chunk_data = chunk.to_dict(orient='records')
    prompt = (
        f"目前正在處理第 {start_idx} 至 {start_idx + len(chunk) - 1} 筆資料（共 {total_records} 筆）。\n"
        f"以下為該批次資料:\n{chunk_data}\n\n"
        "請根據以上資料進行分析，並提供完整的學生讀書建議。"
        "其中請特別注意：\n"
        "  1. 分析學生的科目成績與讀書建議；\n"
        "  2. 請 MultimodalWebSurfer 搜尋外部網站，找出最新的學生讀書建議資訊，\n"
        "     並將搜尋結果整合進回覆中；\n"
        "  3. 最後請提供讀書建議和相關參考資訊。\n"
        "請各代理人協同合作，提供一份完整且具參考價值的建議。"
    )
    
    # 為每個批次建立新的 agent 與 team 實例（team將local_data_agent, local_web_surfer, local_assistant, local_user_proxy建立成團隊）
    ...
    async for event in local_team.run_stream(task=prompt):
        if isinstance(event, TextMessage):
            # 印出目前哪個 agent 正在運作，方便追蹤（印出team裡機器人聊天的內容）
...
async def main():
    ...
    csv_file_path = "8grade-2nd-exam-csv.csv"
    chunk_size = 1000
    chunks = list(pd.read_csv(csv_file_path, chunksize=chunk_size, encoding="utf-8"))
